...
var Job = function( options ) {
	...
	var defaults = {
		...
		cloudfront     : null, // Credentialed AWS-SDK object
...
Job.prototype.create = function() {
	...
	if( (+new Date-this.running) > this.createTimeout * 60000 ) {
		this.log('Timeout: could not create invalidation after ' + this.createTimeout + ' minutes');
		return;
	}
...
Job.prototype.check = function() {
	...
	// See if we've timed out
	if( (+new Date-this.running) > this.checkTimeout * 60000 ) {
